...
from d1_conv_LSTM_utils import hard_sigmoid, get_activationFunc, batch_flatten
...
class Pose_RNN(nn.Module):
    ...
    def forward(self, fv, fv_alter, fi, dec, prev=None):
        ...
        return pose, hc
# 使用ICLR2017所提出的类脑 predictive coding 
class Predictive_Coding_Pose_RNN(nn.Module):
    def __init__(self, opt):
        super(Predictive_Coding_Pose_RNN, self).__init__()
        ...
        #     dropout=opt.rnn_dropout_between,
        #     batch_first=True)
        self.rnn = dimesion_1_conv_LSTM_coding(stack_sizes=(768,768,768,768),R_stack_sizes=(768,768,768,768),output_mode='Ahat0',data_format='channels_first',return_sequences=True)
    ...
    def forward(self, fv, fv_alter, fi, dec, prev=None):
        ...
        # 原本的LSTM需要的
        # out, hc = self.rnn(fused) if prev is None else self.rnn(fused, prev)
        # fused = fused.permute(0,2,1) # [batch_size,768,1]
        fused = fused.squeeze(1)# [b,d]
        out, hc = self.rnn(fused, prev) # 为什么这里out反而是[batch_size,4]呢？应该是1024
        out = self.rnn_drop_out(out)
        pose = self.regressor(out.reshape(out.size(0),-1))

        # hc = (hc[0].transpose(1, 0).contiguous(), hc[1].transpose(1, 0).contiguous())
        return pose.unsqueeze(1), hc
...
class DeepVIO(nn.Module):
    def __init__(self, opt):
        ...
        # predictive coding
        self.Pose_net = Predictive_Coding_Pose_RNN(opt)
    ...
    def forward(self, img, imu, is_first=True, hc=None, temp=5, selection='gumbel-softmax', p=0.5):
        ...
        # 增加
        if hc is None:
            hidden = self.Pose_net.rnn.get_initial_states((fv.size(0), fv.size(1), 768))
        else:
            hidden = hc
        ...
        for i in range(seq_len):
            if i == 0 and is_first:
                ...
                pose, hc = self.Pose_net(fv[:, i:i+1, :], None, fi[:, i:i+1, :], None, hidden)
            else:
                if selection == 'gumbel-softmax':
                    # Otherwise, sample the decision from the policy network
                    p_in = torch.cat((fi[:, i, :], hc[8]), -1)
            ...
            # hc shape[12,16,768] [0-3] R, [4-7] c, [8-11] E
            # hidden = hc[8]
...
class dimesion_1_conv_LSTM_coding(nn.Module):
    """
    ...
    def __init__(self, stack_sizes, R_stack_sizes, 
        ...
        super(dimesion_1_conv_LSTM_coding, self).__init__()
    ...
    def get_initial_states(self, input_shape):
        ...
        base_initial_state = np.sum(base_initial_state, axis = 1)   # (batch_size, 3) noe (batch_size, 768)
        ...
        for sta in states_to_pass:
            for lay in range(layerNum_to_pass[sta]):
                ...
                output_size = stack_size
                reducer = np.zeros((input_shape[-1], output_size))   # (3, output_size)
                initial_state = np.dot(base_initial_state, reducer)                 # (batch_size, output_size)

                if self.data_format == 'channels_first':
                    output_shape = (-1, stack_size)
                else:
                    output_shape = (-1, stack_size)
                # initial_state = torch.from_numpy(np.reshape(initial_state, output_shape)).float().cuda()
                initial_state = torch.from_numpy(np.reshape(initial_state, output_shape)).float().cuda()
                initial_states = initial_states + [initial_state]

        if self.extrap_start_time is not None:
            # initial_states += [torch.IntTensor(1).zero_().cuda()]   # the last state will correspond to the current timestep
            initial_states = initial_states + [torch.IntTensor(1).zero_().cuda()]   # the last state will correspond to the current timestep
        return initial_states # shape[12,16,768]
        ...
        for item in sorted(self.conv_layers.keys()):
            for lay in range(self.num_layers):
                ...
                if item == 'Ahat':
                    ...
                    self.conv_layers['Ahat'].append(nn.Conv1d(in_channels = in_channels,
                                                              out_channels = self.stack_sizes[lay],
                                                              kernel_size = 1,
                                                              stride = 1,
                                                              padding = int((1 - 1) / 2)    # the `SAME` mode (i.e.,(kernel_size - 1) / 2)
                                                              ))
                    ...
                    if self.isNotTopestLayer(lay):   # 这里只是控制一下层数(比其他如Ahat等少一层)
                        ...
                        self.conv_layers['A'].append(nn.Conv1d(in_channels = in_channels,
                                                               out_channels = self.stack_sizes[lay + 1],
                                                               kernel_size = 1,
                                                               stride = 1,
                                                               padding = int((1 - 1) / 2)    # the `SAME` mode
                                                               ))
                    ...
                    if self.isNotTopestLayer(lay):
                        in_channels = in_channels + self.R_stack_sizes[lay + 1]
                    ...
                    self.conv_layers[item].append(nn.Conv1d(in_channels = in_channels,
    ...
    def step(self, A, states):
        ...
        for lay in reversed(range(self.num_layers)):
            inputs = [R_current[lay][:batch_size], E_current[lay][:batch_size]]   # 如果是顶层, R_l的输入只有两个: E_l^t, R_l^(t-1). 即没有高层的R模块的输入项.
            ...
            in_gate     = hard_sigmoid(self.conv_layers['i'][lay](inputs)).squeeze(-1)
            forget_gate = hard_sigmoid(self.conv_layers['f'][lay](inputs)).squeeze(-1)
            cell_gate   = F.tanh(self.conv_layers['c'][lay](inputs)).squeeze(-1)
            out_gate    = hard_sigmoid(self.conv_layers['o'][lay](inputs)).squeeze(-1)
            ...
            c_next = (forget_gate * c_current[lay][:batch_size]) + (in_gate * cell_gate)     # 对应元素相乘
        ...
        for lay in range(self.num_layers):
            tmp_R = R_list[lay][:A.size(0)].unsqueeze(-1)# 3 dim
            Ahat = self.conv_layers['Ahat'][2 * lay](tmp_R)   # Ahat是R的卷积, 故将同层同时刻的R输入. 这里千万注意: 每个`lay`其实对应的是两个组件: 卷积层+非线性激活层, 所以这里需要用(2 * lay)来索引`lay`对应的卷积层, 用(2 * lay + 1)来索引`lay`对应的非线性激活函数层. 下面对A的处理也是一样.
            # Ahat = self.conv_layers['Ahat'][2 * lay + 1](Ahat)      # 勿忘非线性激活.下面对A的处理也是一样.
            # Ahat = F.relu(Ahat)# 增加没有参数
            # Ahat = Ahat.squeeze(-1)
            if lay == 0:
                # Ahat = torch.min(Ahat, self.pixel_max)            # 错误(keras中的表示方式)
                Ahat.squeeze(-1)[Ahat.squeeze(-1) > self.pixel_max] = self.pixel_max        # passed through a saturating non-linearity set at the maximum pixel value
                frame_prediction = Ahat.squeeze(-1)                             # 最低一层的Ahat即为预测输出帧图像
                # if self.output_mode == 'prediction':
                #     break
            ...
            # 在这里之前就出了问题  sum(sum(sum(E_list))).backward(retain_graph=True)
            E_list.append(torch.cat((E_up, E_down), dim = -1).squeeze(-1))
            # 调试梯度所使用
            # sum(sum(E_list)).backward()
            # 如果是想要获取特定的层中特定模块的输出:
            ...
            if self.isNotTopestLayer(lay):
                tmp_E = E_list[lay]
                tmp_E_1 = tmp_E.unsqueeze(-1)
                A = self.conv_layers['A'][2 * lay](tmp_E_1)     # 对E进行卷积+池化之后, 得到同时刻上一层的A, 如果该层已经是最顶层了, 就不用了
        ...
        if self.extrap_start_time is not None:
            states = states + [frame_prediction, (timestep + 1)]
        # # 原始的return
        # return output, states
        # 通过Ahat去预测原图，所以这里应该是Ahat作为我的预测pose，e_list[layer0]作为state
        return output, states
# 将step直接作为新的RNN的forward
    def forward(self, A, states):
        output, hc = self.step(A, states)
        return output, hc
