...
def train(model, optimizer, train_loader, selection, temp, logger, ep, init_state, iter, p=0.5, weighted=False):
    ...
    for i, (imgs, imus, gts, rot, weight) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        # 在这里增加每个epoch的初始化状态  本来原始的情况下这里hc=None
        poses, decisions, probs, _ = model(imgs, imus, is_first=True, hc=init_state, temp=temp, selection=selection, p=p)
        ...
        if i % args.print_frequency == 0: 
            ...
            if args.experiment_name != 'debug':
                ...
                wandb.log({"Epoch": ep, "iters": i, "all_iter": iter, "pose loss": pose_loss.item(),"penalty": penalty, "angle loss": angle_loss.item(), "translation loss": translation_loss.item(), "loss": loss.item()})
...
def main():
    ...
    # torch.cuda.set_device(gpu_ids[0])
    # model.cuda(gpu_ids[0])
    # logger.info('just one gpu is: ' + str(gpu_ids))
    ...
    best = 10000
    # # 应该是只用对第一个epoc初始化
    input_shape = (args.batch_size, args.seq_len, 768)
    init_state = model.module.Pose_net.rnn.get_initial_states(input_shape)
    for ep in range(init_epoch, args.epochs_warmup+args.epochs_joint+args.epochs_fine):
        ...
        # init_state = model.module.Pose_net.rnn.get_initial_states(input_shape)
        states = init_state
        avg_pose_loss, avg_penalty_loss, iter = train(model, optimizer, train_loader, selection, temp, logger, ep, states, iter, p=0.5)
        ...
        if ep%10 == 0 or ep == (args.epochs_warmup+args.epochs_joint+args.epochs_fine - 1):
            # Evaluate the model
